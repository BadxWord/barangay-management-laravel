...
class OfficialsController extends Controller
{
    ...
    public function certificate(Request $request, Officials $official)
    {
       ...
       $barangayID = $this->BarangayID();
       $search = $request->search;
        // Replaced residents_files.name as name as files.name as name
        // See changes in residents_files db table 
       $officials = Officials::where('officials.id', $official->id)
       ->leftJoin('residents_files', function($join) {
           $join
           ->on(DB::raw('1'), '=', DB::raw('1'))
       ...
       ->join('files', 'files.id', '=', 'residents_files.file_id')
       ->select(
        '*',
        'files.name as name',
